...
county_ids_ratios, county_names_id = get_county_ids_ratios(county_ids_ratios_file_path)
print '\nCreating county_ids_ratios [dict] and county_names_id [dict] from \'{0}\'\n'.format(county_ids_ratios_file_path)

print_request = raw_input('Print county_ids_ratios? y or n? ')
if print_request == 'y' :
    print county_ids_ratios

print_request = raw_input('Print county_names_id? y or n? ')
if print_request == 'y' :
    print county_names_id

raw_input('\n\nFile to be parsed: ' + student_list_path)

student_locations_us, student_locations_world, student_locations_bad_states, student_locations_bad_cities, student_locations_errors_other, student_locations_bad_counties, commencement_line_count = get_student_locations(student_list_path, us_states, world_countries, county_names_id)
